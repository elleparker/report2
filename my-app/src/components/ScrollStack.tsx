...
export function MotionScrollStack({items}:{items:{title:string;body:string;}[]}) {
  return (
    <motion.div initial="hidden" whileInView="show" viewport={{once:true}} variants={container}>
      {items.map((it,i)=>(<motion.div key={i} variants={item} className="glass p-4 rounded-lg border border-yellow-400">
        <h4 className="text-xl font-bold mb-2 text-white">{it.title}</h4>
        <p className="text-gray-300">{it.body}</p>
      </motion.div>))}
    </motion.div>
  )
}
